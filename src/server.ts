...
export class Server {
  ...
  private configureRendertronService() {
    const service: RendertronServiceImplementation = {
      screenshot: async (call) => {
        ...
        try {
          return await this.renderer.screenshot(call);
        } catch (err) {
      ...
      serialize: async (call) => {
        ...
        try {
          return await this.renderer.serialize(call);
        } catch (err) {
          throw Server.handleError(err);
        }
      },
  ...
  shutdown() {
    ...
    this.server.shutdown().then(() => this.renderer.shutdown());
  }

  private static handleError(err: unknown): ServerError {
    Server.logger.error(err);
    if (err instanceof PageError) {
      return new ServerError(
        Status.FAILED_PRECONDITION,
        `Requested page returned non 2xx code. (Code: ${err.status})`,
      );
